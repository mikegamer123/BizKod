...
<div class="container-fluid">
        ...
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block sidebar collapse" style="background-color: rgba(196,192,192,0.68)">
            <div class="position-sticky pt-3">
                <ul class="nav flex-column">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="./adminDashboard/events.php">
    ...
    <div class="d-flex justify-content-center">
        <h1 class="h2 py-4"><b>Users</b></h1>
    </div>
    <hr>
    <div>
        <table class="table table-dark table-striped">
            @foreach($users as $user)
            <tr><td>{{$user->nickname}}</td><td>{{$user->first_name}}</td><td>{{$user->last_name}}</td><td>{{$user->email}}</td><td>{{$user->userRole}}</td><td><input type="checkbox" @if($user->new_employeee) checked @endif >New employee</td><td>{{$user->position}}</td><td>{{$user->date_of_birth}}</td><td>{{$user->gender}}</td><td><td><button class="m-auto btn btn-danger">delete</button></td></tr>
            @endforeach
           </table>
    </div>
    </main>
